...
Page({
  ...
  save:function(){
    wx.request({
      url: getApp().globalData.url + '/Api_Applet/UpdatePassWordInfo', //请求接口的url
      method: 'POST', //请求方式
      data: {
...
//蓝牙
  ...
  del(e) {
    ...
    wx.request({
      ...
      method: 'POST', //请求方式
      data: { "deviceId": this.data.deviceID, "btMac": this.data.btMac, "category": 102, "time": this.data.time, "params": { "role": 2, "superPwd": "123456", "value": this.data.valueID, "effectDate": this.data.effectDate + ' ' + '00:00:00', "effectTimes": 255255} },
      // header: {
      //   'content-type': 'application/json' ,// 默认值,
      // },
      complete() {  //请求结束后隐藏 loading 提示框
