...
Page({
  ...
  delFingerprint() {
    ...
    wx.request({
      ...
      method: 'POST', //请求方式
      data: { "deviceId": this.data.deviceID, "btMac": this.data.btMac, "category": 109, "time": time, "params": { "role": 2, "superPwd": "123456", "value": this.data.valueId, "effectDate": this.data.effectDate + ' ' + '00:00:00', "effectTimes": 255255} },
      // header: {
      //   'content-type': 'application/json' ,// 默认值,
      // },
      complete() {  //请求结束后隐藏 loading 提示框
  ...
  responseValue() {
    ...
    for (var i = 0; i < contents.length; i++) {
      ...
      wx.request({
        ...
        method: 'POST', //请求方式
        data: { "deviceId": this.data.deviceID, "btMac": this.data.btMac, "commandId": commandId, "category": 109, "status": 1, "content": contents[i], "time": times, "userId":""},
        // header: {
        //   'content-type': 'application/json' ,// 默认值,
        // },
        complete() {  //请求结束后隐藏 loading 提示框
        ...
        success: res => {
          ...
          //console.log(obj)
          //成功提示
          if (obj.repCode == 50191) {
            //删除成功
            this.delFingerprintpawd();
          } else if (obj.repCode == 50192) {
            wx.showToast({
              title: '删除失败',
              icon: 'loading',
              duration: 1000,
